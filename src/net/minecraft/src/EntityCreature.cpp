...
void EntityCreature::updatePlayerActionState()
{
	...
	if (pathToEntity != nullptr && rand.nextInt(100) != 0)
	{
		...
		if (hasPathPoint)
		{
			...
			{
				// This block's own playerToAttack->posX/posZ finite check is
				// retired: it stayed clean on real hardware, ruling out a
				// stale/dangling pointer here, before moveFlying()'s own
				// finer-grained checks (see Entity.cpp) pinpointed the
				// corruption to its 3-line rescale block instead.
#if PLATFORM_FLOAT_ENTITY_AI_MATH
