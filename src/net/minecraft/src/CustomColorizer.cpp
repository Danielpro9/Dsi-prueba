...
bool CustomColorizer::hasBlockPalette(int_t blockId, int_t metadata)
{
	if (!hasBlockPalettes || blockId < 0 || blockId >= 256)
		return false;
	const int_t md = metadata & 15;
	const int_t palette = blockPalettes[static_cast<size_t>(blockId)][static_cast<size_t>(md)];
	return palette >= 0 && palette < static_cast<int_t>(paletteColors.size()) &&
		!paletteColors[static_cast<size_t>(palette)].empty();
}
